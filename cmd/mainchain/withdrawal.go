...
func RecordReleasedOnMainchain(
	...
	eventChan := make(chan []*mainchain_gateway.MainchainGatewayWithdrew, 10000)
	withdrawalReceiptChan := make(chan map[common.Address][]*withdrawal.WithdrawalReceipt, 10000)
	...
	go func() {
		...
		for {
			...
			case events := <-eventChan:
				withdrawal.SanitizeWithdrewEvents(events, util.ToMap(nwCfg.ExcludeTxHashes), withdrawalReceiptChan)
			}
		}
	}()
